const { CELL_OUT_OF_PLAY } = require('../constants'); // eslint-disable-line no-unused-vars
const { DECAY_TURN_NUMBER } = require('../constants/config'); // eslint-disable-line no-unused-vars
    ...
    boardState.map((row, rowIndex) => {
        ...
        if (rowIndex < decayNum || rowIndex === boardState.length - decayNum) {
            ...
            for (let colIndex = 0; colIndex < row.length && !cellFound; colIndex++) {
                cellFound = row[colIndex] !== CELL_OUT_OF_PLAY;
