...
export function* createGame() {
    ...
    yield sendRequest({
        ...
        onFailure: function* failure(err) {
            ...
            yield put({type: GAME_CREATE_FAILURE, message: `Failed to connect to Coherent Chaos: ${err}`});
        }
    });
}
...
export function* joinGameAttempt(action) {
    ...
    yield sendRequest({
        ...
        onFailure: function* failure(err) {
            ...
            yield put({type: GAME_JOIN_ATTEMPT_FAILURE, message: `Failed to connect to Coherent Chaos: ${err}`});
        }
    });
}
...
export function* joinGamePickPlayer(action) {
    ...
    yield sendRequest({
        ...
        onFailure: function* failure(err) {
            ...
            yield put({type: GAME_JOIN_PICK_PLAYER_FAILURE, message: `Failed to connect to Coherent Chaos: ${err}`});
        }
    });
}
